...
const MeetTheFounders = () => {
  ...
  return (
    <section id="founders" className="w-full py-20 bg-gradient-to-r from-gray-900 to-black text-white rounded-3xl shadow-2xl">
      <h1 className="text-center text-blue-400 text-[19px] lg:text-[50px] font-ubuntu font-semibold mb-12 tracking-wide">Meet the Founders</h1>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 max-w-7xl mx-auto px-10">
        {foundersInfo.map((founder, index) => (
          ...
          >
            <img src={founder.image} alt={founder.name} className="rounded-full w-[250px] h-[250px] mx-auto border-4 border-blue-400 mb-6 transform hover:scale-110 transition-transform duration-300" />
            <h2 className="text-[28px] text-blue-400 font-ubuntu font-semibold mb-2">{founder.name}</h2>
            <p className="text-[20px] mb-4">{founder.role}</p>
            <p className="text-[16px] text-blue-400 mb-6 font-ubuntu">{founder.about}</p>
            <div className="flex justify-center items-center gap-6">
              <a href={founder.social.linkedin} target="_blank" rel="noopener noreferrer">
                <FaLinkedin className="text-blue-400 text-[30px] hover:text-white transition-colors duration-300" />
              </a>
              <a href={founder.social.twitter} target="_blank" rel="noopener noreferrer">
                <FaTwitter className="text-blue-400 text-[30px] hover:text-white transition-colors duration-300" />
              </a>
              <a href={founder.social.github} target="_blank" rel="noopener noreferrer">
                <FaGithub className="text-blue-400 text-[30px] hover:text-white transition-colors duration-300" />
              </a>
              <a href={founder.social.instagram} target="_blank" rel="noopener noreferrer">
                <FaInstagram className="text-blue-400 text-[30px] hover:text-white transition-colors duration-300" />
              </a>
            </div>
          </div>
        ))}
